...
): Promise<{ percentage: number; reasoning: string; highlights: string }> => { // Simplified return type
...
`;
  ...
  try {
    const result: { percentage: number; reasoning: string; highlights?: string } = JSON.parse(jsonString);
    ...
    const finalHighlights = (result.highlights ?? '').replace(/\n{3,}/g, '\n\n').trim();

    return { percentage: result.percentage, reasoning: finalReasoning, highlights: finalHighlights };
  } catch (e) {
