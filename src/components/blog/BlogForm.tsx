...
export const BlogForm = ({ editingPost, galleryImages, uniqueTags, onSubmit, onCancel }: BlogFormProps) => {
  ...
  useEffect(() => {
    if (isEditorOpen) {
      setEditorContent(form.getValues('content'));
    }
  }, [isEditorOpen, form]);

  const handleSaveFromEditor = () => {
    form.setValue('content', editorContent, { shouldValidate: true, shouldDirty: true });
    ...
    <>
              ...
              <FormField control={form.control} name="content" render={({ field }) => (
                <FormItem>
                  <div className="flex items-center justify-between">
                    ...
                    <Button type="button" variant="ghost" size="icon" onClick={() => setIsEditorOpen(true)}>
                      <Maximize className="h-4 w-4" />
                      <span className="sr-only">Open Rich Text Editor</span>
                    </Button>
                  </div>
                  <FormControl><Textarea placeholder="Write your full article here..." className="min-h-[200px]" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
      ...
      <Dialog open={isEditorOpen} onOpenChange={setIsEditorOpen}>
        <DialogContent className="max-w-5xl w-full h-[90vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>Rich Text Post Editor</DialogTitle>
            <DialogDescription>
              Edit your post content here. Changes will be saved to the form when you click "Save & Close".
            </DialogDescription>
          </DialogHeader>
          <div className="flex-grow overflow-y-auto -mx-6 px-6 border-y">
            <PostEditor
          ...
          <DialogFooter className="pt-4">
            ...
            <Button onClick={handleSaveFromEditor}>Save & Close</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
